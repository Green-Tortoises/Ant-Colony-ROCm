...
def main():
    ...
    original_df = pd.read_csv("heloc_dataset_v1.csv", sep=';') 
    
    dataframe = pd.read_csv("heloc_dataset_v1.csv", sep=';')
    
    classes = dataframe["PercentTradesWBalance"]
    dataframe = dataframe.drop(columns=["PercentTradesWBalance"])
    initial_pheromone = 1
    Q = 1
